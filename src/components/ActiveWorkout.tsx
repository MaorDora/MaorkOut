...
export default function ActiveWorkout({ workout, onClose, onComplete }: ActiveWorkoutProps) {
  ...
  const handleNext = () => {
    ...
    if (currentExercise.restSeconds > 0 && currentExerciseIndex < workout.exercises.length - 1) {
      setIsResting(true);
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-[60] flex flex-col">
          ...
          {isResting ? (
            ...
            >
              ...
              <div className="glass p-6 rounded-3xl border border-white/10">
                <div className="text-slate-400 text-sm mb-2">התרגיל הבא</div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="glass p-6 rounded-3xl text-center border border-white/5">
                  <div className="text-4xl font-bold text-white mb-1">{currentExercise.sets}</div>
                  <div className="text-slate-500 text-xs font-bold uppercase tracking-widest">סטים</div>
                </div>
                <div className="glass p-6 rounded-3xl text-center border border-white/5">
              ...
              <div className="glass p-6 rounded-3xl border border-white/5">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-slate-400 font-medium">היסטוריה</span>
                  <span className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-1 rounded-md border border-blue-500/20">אימון אחרון</span>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm items-center p-3 rounded-xl bg-white/5">
                    <span className="text-slate-300">סט 1</span>
                    <span className="font-mono text-white font-bold">12 חזרות • 20 ק״ג</span>
                  </div>
                  <div className="flex justify-between text-sm items-center p-3 rounded-xl bg-white/5">
                    <span className="text-slate-300">סט 2</span>
                    <span className="font-mono text-white font-bold">10 חזרות • 22.5 ק״ג</span>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      {/* Footer Controls */}
      <div className="p-6 border-t border-white/5 bg-slate-950/80 backdrop-blur-xl flex gap-4 relative z-10 pb-safe">
        <button 
          onClick={() => setCurrentExerciseIndex(Math.max(0, currentExerciseIndex - 1))}
          disabled={currentExerciseIndex === 0 || isResting}
        ...
        <button 
          onClick={handleNext}
          className="flex-1 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-bold text-lg transition-all shadow-[0_0_30px_rgba(37,99,235,0.3)] flex items-center justify-center"
        >
          {isResting ? 'סיים מנוחה' : (currentExerciseIndex === workout.exercises.length - 1 ? 'סיים אימון' : 'הושלם, לתרגיל הבא')}
        </button>
      </div>
    </div>
  );
